		...
		for (int j=0; j<input_channel ; j++){
			for (int k=0; k<conv_height ; k++) {
				for (int l=0; l<conv_width ; l++) {
					new_weights[i][(conv_height*conv_width)*j + k*conv_width + l] = cnn_weights[i][j][k][l];
				}
			}
		}
	}
	...
	for (int i=0; i<input_channel; i++){
		for (int j=0; j<input_height - conv_height + 1; j++){
			for (int k=0; k<input_width - conv_width + 1; k++){
				//new_input[(conv_height*conv_width)*input_channel + (j * input_width) + k][ ] =  
				for (int l=0; l<conv_height ; l++){
					for (int m=0; m<conv_width ; m++){
						 new_inputs[(conv_height*conv_width)*(i) + (conv_width*l + m) ] [(j*(input_width - conv_width + 1)) + k ] = inputs[i][j+l][k+m];
					}
				}
